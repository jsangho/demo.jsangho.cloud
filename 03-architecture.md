...
| 데이터 저장소 | 좌표 기반 거리 계산, 이력 조회, 장면 색인 검색을 감당해야 한다 |
...
**판정은 근거 장면과 함께 저장한다.** [제 6 장]({{ '/06-verification/' | relative_url }})의 검증은 근거 없이 성립하지 않는다. 판정 레코드가 근거 장면을 참조하지 않으면 저장 단계에서 거부한다. 이용자가 이의를 제기했을 때 무엇을 보고 판정했는지 되짚을 수 있어야 한다.
