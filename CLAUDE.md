...
본문은 **구현물이 없는 상태의 설계 서술**이다. 화면 캡처·API 경로·실측치는 넣지 않았고, 팀 결정이나 실측이 필요한 값은 본문에 `〔확인 필요: …〕`로 표시해 두었다(현재 42곳). 이 표기를 지어낸 값으로 채우지 않는다 — 정해진 값을 받아서 채운다.
...
- **`〔확인 필요〕` 42곳** — 종목 범위, 실력 등급 구간, 순위 가중치, 취소·제재 기준, 기술 스택, 비기능 목표치 등. 팀 결정 대기
