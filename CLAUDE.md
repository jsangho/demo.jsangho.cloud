...
| `brand` | `_config.yml` | 상단 헤더 좌측, `report` 머리말 — **사업명이 아니라 팀명**(`Super-Sub`) |

좁은 자리는 사업명 약칭이 아니라 팀명으로 통일했다. 사업명이 바뀌어도 이 자리들은 그대로다.
...
| 장 | 상태 | 비고 |
|---|---|---|
| 1. 사업 개요 | 초안 | 2~6장 요약. 제외 범위(결제·구장 예약·리그)와 측정 지표 4개 포함 |
| 2. 요구사항 분석 | 초안 | UR-01~08, FR-11~42(14개), NFR-01~07 |
| 3. 시스템 설계 | 초안 | 기술 스택은 기준·검토 영역만, 확정 보류 |
| 4. AI 에이전트 설계 | 초안 | 모델 후보·단가는 2026-06 기준, 최종 선정 보류 |
| 5. 용병 구인 기능 개발 | 초안 | |
| 6. 실력 검증 기능 개발 | 초안 | |
| 7. 시험 및 검증 결과 | **골격만** | TC-11~42(21개)·NT-01~07과 판정 기준 확정, 결과 칸은 비어 있음 |
| 8. 결론 및 향후 과제 | **미작성** | 7장 시험 결과가 나와야 쓸 수 있다 |

본문은 **구현물이 없는 상태의 설계 서술**이다. 화면 캡처·API 경로·실측치는 넣지 않았고, 팀 결정이나 실측이 필요한 값은 본문에 `〔확인 필요: …〕`로 표시해 두었다(현재 35곳). 이 표기를 지어낸 값으로 채우지 않는다 — 정해진 값을 받아서 채운다.
...
- **`〔확인 필요〕` 35곳** — 종목 범위, 실력 등급 구간, 순위 가중치, 취소·제재 기준, 기술 스택, 비기능 목표치 등. 팀 결정 대기
