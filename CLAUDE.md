...
- 사업명(국문): 생활체육 경기 영상을 멀티모달로 분석하여 용병을 구인하고 RAG 검증을 자동화하는 플랫폼 (Hit Rate 90%, Recall@5 90% 달성)
- 사업명(영문): Multimodal Analysis of Amateur Sports Match Videos for Substitute Player Scouting and Automated RAG Verification
- 축약 제목: 멀티모달 용병 스카우팅 & RAG 검증 플랫폼
...
| `report` | 본문 각 장 | 있음 |
| `page` | minima 기본. 현재 쓰는 페이지 없음 | 있음 |

본문 페이지는 `report`를 쓴다. 목차·문서 정보·스프린트 기록 같은 부속 페이지도 `report`를 쓴다 — `page`는 minima 기본이라 보고서 타이포그래피가 적용되지 않는다. `chapter`가 없는 페이지는 머리말에 `제 N 장` 대신 팀명이 나오고 이전/다음 네비가 붙지 않는다.
...
국문 사업명이 길어(공백 포함 38자) 좁은 자리에는 그대로 못 넣는다. 사업명이 바뀌면 **아래 세 곳을 함께** 고친다(`brand`는 팀명이라 별개다).
...
표지 제목은 `index.md`의 `title_break`에 적힌 어절들 **뒤에서** 차례로 끊긴다. 어절 수 + 1 줄이 된다. 레이아웃이 `title` 하나를 쪼개 쓰므로 제목 문자열이 중복되지 않는다.

```yaml
title_break:      # 반드시 목록으로 쓴다. 문자열로 쓰면 Liquid 반복이 돌지 않는다
  - 분석하여
  - 플랫폼
```

제목에 없는 어절은 건너뛰고, `title_break`가 없으면 한 덩어리로 렌더된다. 같은 어절이 제목에 두 번 나오면 **첫 번째**에서 끊긴다.

줄 수는 CSS와 맞물려 있다. `.cover__title`의 `font-size` 상한(2.1rem)과 `.cover__inner`의 `max-width`(48rem)는 **가장 긴 줄이 한 줄로 들어가도록** 잡은 값이다. 둘 중 하나만 건드리면 그 줄이 접혀 줄 수가 늘어나므로, 크기를 키우려면 폭도 같이 키우고 브라우저에서 확인한다. 제목이 바뀌면 가장 긴 줄도 바뀌므로 이 값들을 다시 맞춘다.
...
- **본문 장 페이지 생성**(`01-overview.md` ~ `09-conclusion.md`) 및 배포 — 2026-08-21
...
- **본문 1~6장 초안 작성 + 7장 골격** — 2026-08-21. 아래 '본문 작성 현황' 참조

### 본문 작성 현황

| 장 | 상태 | 비고 |
|---|---|---|
| 1. 사업 개요 | 초안 | 2~6장 요약. 제외 범위(결제·구장 예약·리그)와 측정 지표 5개 포함 |
| 2. 요구사항 분석 | 초안 | UR-01~08, FR-11~42(14개), NFR-01~08 |
| 3. 시스템 설계 | 초안 | 기술 스택은 기준·검토 영역만, 확정 보류 |
| 4. AI 에이전트 설계 | 초안 | 영상 분석(프레임 추출)·매칭·검증 3종. 모델 후보·단가는 2026-06 기준, 최종 선정 보류 |
| 5. 용병 구인 기능 개발 | 초안 | |
| 6. 실력 검증 기능 개발 | 초안 | RAG 근거 검색 기반. Hit Rate·Recall@5 목표 90% |
| 7. 시험 및 검증 결과 | **골격만** | TC-11~42(22개)·NT-01~08과 판정 기준 확정, 결과 칸은 비어 있음 |
| 8. 개발 일정 및 추진 체계 | 초안 | 스크럼 5스프린트, 주차별 진행, 칸반 보드, 역할 분담(RACI·교차 리뷰), 위험 R-01~08 |
| 9. 결론 및 향후 과제 | **골격만** | 결과·지표 표는 비어 있고, 설계 단계에서 드러난 한계 5가지와 향후 계획은 작성 |

8장은 2026-08-21에 추가하면서 결론을 8 → 9로 밀었다. 장을 끼워 넣으면 **번호·슬러그·본문의 `제 N 장` 상호 참조를 모두 함께** 고쳐야 한다. 확인 명령은 아래와 같다.

```bash
grep -rn '제 [0-9] 장' 0*.md          # 상호 참조 대상 번호
grep -rn '0[0-9]-[a-z]*/' toc.md      # 목차 링크 슬러그
```

본문의 검증 방식은 **경기 영상 멀티모달 분석 + RAG 근거 검색**이다. 2026-08-21에 사업명이 바뀌면서 기존의 '상대 팀 평가(레드팀) 기반' 서술을 이 방향으로 전면 교체했다(1~8장). 평가 기반 서술이 남아 있으면 사업명과 어긋난 것이므로 고친다.

본문은 **구현물이 없는 상태의 설계 서술**이다. 화면 캡처·API 경로·실측치는 넣지 않았고, 팀 결정이나 실측이 필요한 값은 본문에 `〔확인 필요: …〕`로 표시해 두었다(현재 44곳). 이 표기를 지어낸 값으로 채우지 않는다 — 정해진 값을 받아서 채운다.

```bash
grep -rn '〔확인 필요' 0*.md about.markdown sprint-1.md   # 남은 미확정 값 목록
```

장 사이 상호 참조는 `{{ '/NN-slug/' | relative_url }}` 링크로 걸려 있다. 어느 장의 판단을 다른 장에서 근거로 삼을 때는 링크를 함께 단다(예: 5장의 "자동 확정 없음"은 2장 UR-04를 근거로 든다).

### 본문 외 페이지

| 파일 | 주소 | 용도 |
|---|---|---|
| `index.md` | `/` | 표지 (`cover` 레이아웃) |
| `toc.md` | `/toc/` | 목차. 하단에 표지 복귀 링크 |
| `about.markdown` | `/about/` | 문서 정보. 사업 정보·장 구성·읽는 법·사이트 설명 |
| `sprint-1.md` | `/sprint-1/` | 스프린트 1 일자별 진행 기록. 8장 스프린트 표의 `S1`에서 링크된다 |

`about.markdown`의 사업 정보 표는 값을 직접 적지 않고 `index.md`의 front matter를 Liquid로 읽어 온다(`site.pages | where: "permalink", "/"`). 팀원·기간·주소가 바뀌면 `index.md`만 고치면 표지와 문서 정보가 함께 따라온다.

스프린트 기록은 스프린트마다 `sprint-N.md`로 추가하고 8장 표의 해당 스프린트를 링크한다. 일자별 표는 **실제로 한 작업**으로 채운다 — 계획을 옮겨 적고 완료로 표시하지 않는다.

### 남은 작업

- **7·9장 결과 미기록** — 시험 수행 후 채운다. 위 표 참조
- **`〔확인 필요〕` 44곳** — 종목 범위, 실력 등급 구간, 순위 가중치, 취소·제재 기준, 기술 스택, 비기능 목표치 등. 팀 결정 대기
