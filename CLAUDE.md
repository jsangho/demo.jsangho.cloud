...
| 8. 개발 일정 및 추진 체계 | 초안 | 스크럼 5스프린트, 칸반 보드, 역할 분담, 위험 R-01~07. 정어진은 참여 여부 미정이라 비워 둠 |
| 9. 결론 및 향후 과제 | **미작성** | 7장 시험 결과가 나와야 쓸 수 있다 |
...
본문은 **구현물이 없는 상태의 설계 서술**이다. 화면 캡처·API 경로·실측치는 넣지 않았고, 팀 결정이나 실측이 필요한 값은 본문에 `〔확인 필요: …〕`로 표시해 두었다(현재 37곳). 이 표기를 지어낸 값으로 채우지 않는다 — 정해진 값을 받아서 채운다.

```bash
grep -n '〔확인 필요' 0*.md      # 남은 미확정 값 목록
```
...
- **8장 미작성**, **7장 결과 미기록** — 위 표 참조
- **`〔확인 필요〕` 37곳** — 종목 범위, 실력 등급 구간, 순위 가중치, 취소·제재 기준, 기술 스택, 비기능 목표치 등. 팀 결정 대기
